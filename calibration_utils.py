...
def estimate_pose_and_filter(camData: CameraData, corners, ids,
  ...
  while len(objects) < len(objpoints[:, 0, 0]) * camData['min_inliers']:
    if ini_threshold > camData['max_threshold']:
      break
    ret, rvec, tvec, objects = cv2.solvePnPRansac(
        objpoints,
        corners,
        camData['intrinsics'],
        camData['dist_coeff'],
    ...
    ret, rvec, tvec = cv2.solvePnP(imgpoints2, all_corners2,
                                   camData['intrinsics'],
                                   camData['dist_coeff'])
  ...
  if ids.size > 0 and corners.size > 0:  # TODO : Try to remove the np reshaping
    ...
    return corners2[valid_mask].reshape(-1, 1, 2), valid_ids.reshape(
        -1, 1).astype(np.int32), corners2[removed_mask]
...
def detect_charuco_board(image: np.array, board: CharucoBoard):
  ...
  markers, marker_ids, rejectedImgPoints = cv2.aruco.detectMarkers(
      image, board.dictionary, parameters=arucoParams)  # First, detect markers
  ...
  if len(marker_corners) > 0:
    num_corners, corners, ids = cv2.aruco.interpolateCornersCharuco(
        marker_corners, marker_ids, image, board.board, minMarkers=1)
    if corners is not None and ids is not None and len(corners) > 3:
      corners = cv2.cornerSubPix(image,
                                 corners,
                                 winSize=(5, 5),
                                 zeroZone=(-1, -1),
                                 criteria=criteria)
...
def get_features(config: CalibrationConfig, camData: CameraData) -> CameraData:
  ...
  camData['intrinsics'] = np.array([[f, 0.0, camData['size'][0] / 2],
  ...
  max_threshold = 75 + config.initialMaxThreshold * (
      camData['hfov'] / 30 + camData['size'][1] / 800 * 0.2)
  threshold_stepper = int(1.5 *
                          (camData['hfov'] / 30 + camData['size'][1] / 800))
  if threshold_stepper < 1:
    threshold_stepper = 1
  min_inliers = 1 - config.initialMinFiltered * (
      camData['hfov'] / 60 + camData['size'][1] / 800 * 0.2)
...
@parallel_function
def calibrate_charuco(camData: CameraData, allCorners, allIds,
                      dataset: Dataset):
  ...
  for i, ids in enumerate(allIds):
    allIds[i] = ids.reshape(-1, 1).astype(np.int32)
...
@parallel_function
def filter_features_fisheye(camData: CameraData, intrinsic_img, all_features,
                            all_ids):
  f = camData['size'][0] / (2 * np.tan(np.deg2rad(camData["hfov"] / 2)))
  ...
  if camData["name"] in intrinsic_img:
    ...
    all_features, all_ids, filtered_images = remove_features(
        filtered_features, filtered_ids, intrinsic_img[camData["name"]],
        image_files)
  else:
...
@parallel_function
def calibrate_ccm_intrinsics_per_ccm(config: CalibrationConfig,
                                     camData: CameraData, dataset: Dataset):
  start = time.time()
  print('starting calibrate_wf')
  ret, cameraIntrinsics, distCoeff, _, _, filtered_ids, filtered_corners, size, coverageImage, all_corners, all_ids = calibrate_wf_intrinsics(
  ...
  print("Reprojection error of {0}: {1}".format(camData['name'], ret))
...
@parallel_function
def calibrate_stereo_perspective(config: CalibrationConfig, obj_pts,
                                 allLeftCorners, allRightCorners,
                                 leftCamData: CameraData,
                                 rightCamData: CameraData):
  cameraMatrix_l, distCoeff_l, cameraMatrix_r, distCoeff_r, left_distortion_model = leftCamData[
      'intrinsics'], leftCamData['dist_coeff'], rightCamData[
          'intrinsics'], rightCamData['dist_coeff'], leftCamData[
              'distortion_model']
  specTranslation = leftCamData['extrinsics']['specTranslation']
  rot = leftCamData['extrinsics']['rotation']

  t_in = np.array(
      [specTranslation['x'], specTranslation['y'], specTranslation['z']],
      dtype=np.float32)
  r_in = Rotation.from_euler('xyz', [rot['r'], rot['p'], rot['y']],
  ...
  R_l, R_r, P_l, P_r, Q, validPixROI1, validPixROI2 = cv2.stereoRectify(
      cameraMatrix_l, distCoeff_l, cameraMatrix_r, distCoeff_r, None, R,
      T)  # , alpha=0.1
  # self.P_l = P_l
  # self.P_r = P_r
...
@parallel_function
def calibrate_stereo_perspective_per_ccm(config: CalibrationConfig, obj_pts,
                                         left_corners_sampled,
                                         right_corners_sampled, left_cam_info,
                                         right_cam_info):
...
def calibrate_stereo_fisheye(config: CalibrationConfig, obj_pts,
  ...
  (ret, M1, d1, M2, d2, R, T), E, F = cv2.fisheye.stereoCalibrate(
      ...
      None,
      flags=flags,
      criteria=config.stereoCalibCriteria), None, None
  r_euler = Rotation.from_matrix(R).as_euler('xyz', degrees=True)
  print(f'Reprojection error is {ret}')
  isHorizontal = np.absolute(T[0]) > np.absolute(T[1])

  R_l, R_r, P_l, P_r, Q = cv2.fisheye.stereoRectify(cameraMatrix_l,
                                                    distCoeff_l,
                                                    cameraMatrix_r,
                                                    distCoeff_r,
                                                    None,
...
@parallel_function
def find_stereo_common_features(leftCorners, leftIds, rightCorners, rightIds,
                                board: CharucoBoard):
  left_corners_sampled = []
  right_corners_sampled = []
  obj_pts = []
  ...
  if failed > len(leftIds) / 3:
    raise RuntimeError(
        'More than 1/3 of images had less than 6 common features found')
...
def undistort_points_perspective(allCorners, camInfo):
  return [
      cv2.undistortPoints(np.array(corners),
                          camInfo['intrinsics'],
                          camInfo['dist_coeff'],
                          P=camInfo['intrinsics']) for corners in allCorners
  ]
...
def undistort_points_fisheye(allCorners, camInfo):
  return [
      cv2.fisheye.undistortPoints(np.array(corners),
                                  camInfo['intrinsics'],
                                  camInfo['dist_coeff'],
                                  P=camInfo['intrinsics'])
      for corners in allCorners
  ]


@parallel_function
def calculate_epipolar_error(left_cam_info: CameraData,
                             right_cam_info: CameraData, left_cam: Dataset,
                             right_cam: Dataset, board_config, extrinsics):
  if extrinsics[0] == -1:
    return -1, extrinsics[1]
  stereoConfig = None
  if 'stereo_config' in board_config:
    leftCamName = board_config['cameras'][board_config['stereo_config']
                                          ['left_cam']]['name']
    rightCamName = board_config['cameras'][board_config['stereo_config']
                                           ['right_cam']]['name']
  ...
  if PER_CCM and EXTRINSICS_PER_CCM:
    scale = (
        (left_cam_info['intrinsics'][0][0] * right_cam_info['intrinsics'][0][0]
         + left_cam_info['intrinsics'][1][1] *
         right_cam_info['intrinsics'][1][1]) / 2)
    print(f"Epipolar error {extrinsics[0]*np.sqrt(scale)}")
    left_cam_info['extrinsics']['epipolar_error'] = extrinsics[0] * np.sqrt(
        scale)
  else:
...
def draw_corners(charuco_corners, displayframe):
  for corners in charuco_corners:
    color = (int(np.random.randint(0, 255)), int(np.random.randint(0, 255)),
...
def features_filtering_function(rvecs,
                                tvecs,
                                cameraMatrix,
                                distCoeffs,
                                filtered_corners,
  ...
  for i, (corners, ids) in enumerate(zip(filtered_corners, filtered_id)):
    if ids is not None and corners.size > 0:
      ...
      all_ids.append(valid_ids.reshape(-1, 1).astype(
          np.int32))  # Reshape and store as array of arrays
      ...
      all_points.append(
          imgpoints2[valid_mask])  # Collect valid points for calibration
      all_corners.append(corners2[valid_mask].reshape(
          -1, 1, 2))  # Collect valid corners for calibration
      ...
      rms_error = np.sqrt(total_error_squared /
...
def camera_pose_charuco(objpoints: np.array,
                        corners: np.array,
                        ids: np.array,
                        K: np.array,
                        d: np.array,
...
def undistort_visualization(self, img_list, K, D, img_size, name):
  for index, im in enumerate(img_list):
    ...
    if self._cameraModel == 'perspective':
      ...
      # print(f'K scaled is \n {kScaled} and size is \n {img_size}')
      # print(f'D Value is \n {D}')
      map1, map2 = cv2.initUndistortRectifyMap(K, D, np.eye(3), kScaled,
...
def filter_corner_outliers(config: CalibrationConfig, allIds, allCorners,
  ...
  for i in range(len(allIds)):
    ...
    if config.cameraModel == "fisheye":
      ...
      errs = compute_reprojection_errors(objpts, corners, camera_matrix,
                                         distortion_coefficients,
                                         rotation_vectors[i],
                                         translation_vectors[i])
    ...
    if n_offending_pts > 0 and n_offending_pts < len(corners) / 5:
      print(
          f"removing {n_offending_pts} offending points with errs {errs[offending_pts_idxs]}"
      )
...
def calibrate_camera_charuco(allCorners, allIds, imsize, distortion_flags,
                             cameraIntrinsics, distCoeff, dataset: Dataset):
  """
    ...
    while not success:
      ...
      for obj_pts, corners in zip(obj_points, filtered_corners):
        ...
        for obj_pt, corner in zip(obj_pts, corners):
          check_x = corner[0, 0] > imsize[0] * (1 - crop) and corner[
              0, 0] < imsize[0] * crop
          check_y = corner[0, 1] > imsize[1] * (1 - crop) and corner[
              0, 1] < imsize[1] * crop
      ...
      try:
        res, K, d, rvecs, tvecs = cv2.fisheye.calibrate(obj_points_limited,
                                                        corners_limited,
                                                        None,
                                                        cameraMatrixInit,
                                                        distCoeffsInit,
        ...
        if crop > 0.7:
          ...
          raise Exception(
              "Calibration failed: Tried maximum crop factor and still no success"
          )
    if success:
      ...
      try:
        ...
        print(
            f"Failed the full res calib, using calibration with crop factor {crop}"
        )
...
def calibrate_camera(config,
  ...
  for dataset in uniqueDatasets:
    ...
    if len(dataset.allCorners) and len(dataset.allIds):
      ...
      raise RuntimeError(
          f'Dataset \'{dataset.name}\' doesn\'t contain corners or images')
  ...
  for left, right in extrinsicPairs:
    ...
    if PER_CCM and EXTRINSICS_PER_CCM:
      if left_cam_info['calib_model'] == "perspective":
        left_corners_sampled = undistort_points_perspective(
            left_corners_sampled, left_cam_info)
        right_corners_sampled = undistort_points_perspective(
            right_corners_sampled, right_cam_info)
      else:
        left_corners_sampled = undistort_points_fisheye(
            left_corners_sampled, left_cam_info)
        right_corners_sampled = undistort_points_fisheye(
            right_corners_sampled, right_cam_info)

      extrinsics = calibrate_stereo_perspective_per_ccm(
          config, obj_pts, left_corners_sampled, right_corners_sampled,
          left_cam_info, right_cam_info)
    else:
      if camera_model == 'perspective':
        extrinsics = calibrate_stereo_perspective(config, obj_pts,
                                                  left_corners_sampled,
                                                  right_corners_sampled,
                                                  left_cam_info,
                                                  right_cam_info)
      elif camera_model == 'fisheye':
        extrinsics = calibrate_stereo_fisheye(config, obj_pts,
                                              left_corners_sampled,
                                              right_corners_sampled,
                                              left_cam_info, right_cam_info)
  ...
  def calibrate(self,
    ...
    for a, b in extrinsicPairs:
      if len(a.allIds) != len(b.allIds):
        print(
            'Not all dataset for extrinsic calibration have the same number of images'
        )
        raise RuntimeError(
            'Not all dataset for extrinsic calibration have the same number of images'
        )  # TODO : This isn't thorough enough
